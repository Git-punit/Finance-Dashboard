...
export function EditWidgetModal({ isOpen, onClose, widget }: EditWidgetModalProps) {
    ...
    const [dataKey, setDataKey] = useState(widget.dataKey || '');
    const [refreshInterval, setRefreshInterval] = useState(widget.refreshInterval);
    const [format, setFormat] = useState(widget.format || 'number');
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setDataKey(widget.dataKey || '');
            setRefreshInterval(widget.refreshInterval);
            setFormat(widget.format || 'number');
        }
    }, [isOpen, widget]);
            ...
            <div className="w-full max-w-lg bg-slate-900 border border-slate-700/50 rounded-2xl shadow-2xl scale-100 animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
                <div className="flex justify-between items-center p-6 border-b border-slate-800">
                    <div>
                        ...
                        <p className="text-slate-400 text-sm">Update configuration for {title}.</p>
                    </div>
                ...
                <div className="p-6 overflow-y-auto">
                    <form id="edit-widget-form" onSubmit={handleSubmit} className="space-y-5">
                        <div>
